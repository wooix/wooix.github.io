...
<span id="history역사-더-많은-표본에서-더-나은-연습문제로"></span>
...
<details class="paper-reading-list" id="discussion-commentary">
<summary>토론 해설 펼치기 · 5개 질문</summary>

1. **실제 질문이 충분하다면, 기존 질문에 라벨이나 응답을 붙이는 것부터 시작하는 편이 합리적입니다.**

   부족한 것이 질문인지, 학습에 사용할 정답인지 먼저 구분해야 합니다. 기존 질문에 사람이 라벨을 붙이거나, 모델이 응답 초안을 생성한 뒤 문서·규칙·사람의 검토로 검증하면 실제 사용자의 요구를 반영한 학습 데이터를 만들 수 있습니다. 모델이 붙인 답은 검증 전까지 ‘정답 후보’로 취급해야 합니다.

   새 질문 생성은 기존 질문에 없는 주제나 능력, 드문 상황을 보완할 때 유용합니다. 따라서 **기존 입력의 정답 확보를 우선하고, 부족한 영역에 질문 생성을 쓰는 방식**이 적절합니다.

2. **질문을 100가지 문체로 바꾸면 표현의 다양성은 늘지만, 요구하는 능력의 다양성은 반드시 늘지 않습니다.**

   표현의 다양성에는 어휘, 문장 구조, 존댓말·반말, 길이, 구어체 등이 포함됩니다. 같은 의도를 여러 방식으로 이해하는 능력을 기르는 데 도움이 됩니다.

   반면 “두 수의 합을 구하라”를 100가지로 바꿔도 요구 능력은 여전히 덧셈입니다. 추론 단계, 필요한 지식, 판단 조건이 그대로라면 과제의 다양성은 거의 늘지 않습니다. 능력까지 다양하게 하려면 비교, 원인 분석, 다단계 계산, 불충분한 정보 식별처럼 **수행해야 할 작업 자체를 바꿔야 합니다.** 다만 문체 변환 과정에서 모호함이나 새 조건이 추가됐다면, 단순한 표현 변경을 넘어 과제도 달라졌는지 확인해야 합니다.

3. **생성 모델과 평가 모델이 동의한다는 사실만으로 정답이라고 판단할 수는 없습니다.**

   두 모델은 비슷한 학습 데이터나 편향 때문에 같은 오류를 낼 수 있습니다. 다른 모델을 쓰더라도 판단의 독립성이 보장되지는 않습니다. 합의는 검토 우선순위를 정하는 신호로 활용할 수 있지만, 정답의 증명은 아닙니다.

   필요한 외부 근거는 과제에 따라 다릅니다.

   - 사실 질문: 신뢰할 수 있는 원문, 공식 문서, 검증된 데이터베이스
   - 수학·논리: 계산 결과, 증명, 반례 검토
   - 코드: 실행 결과, 독립적으로 작성한 테스트, 명세 충족 여부
   - 전문 분야: 관련 지침과 전문가 검토
   - 주관적 응답: 명시적인 평가 기준과 실제 사용자·복수 평가자의 판단

   특히 주관적 과제에서는 단일한 정답보다 **어떤 기준을 얼마나 충족하는지**를 평가해야 합니다.

4. **예외 질문을 많이 학습시키는 것은 실제 분포와의 차이이면서, 목적에 따라 필요한 설계일 수 있습니다.**

   실제보다 예외 사례의 비중을 높이면 학습 분포는 달라집니다. 하지만 드물어도 실패 비용이 큰 상황을 잘 처리하는 것이 목표라면, 의도적인 과대표집이 타당할 수 있습니다.

   중요한 것은 목표와 평가 기준을 함께 정하는 것입니다. 일반적인 사용 품질이 목표라면 실제 질문 비율을 반영한 평가가 필요합니다. 예외 대응의 안정성이 목표라면 예외 유형별 성공률과 심각한 실패율도 따로 측정해야 합니다. 동시에 예외 학습 때문에 정상 질문을 과도하게 거절하거나 잘못 경고하는지도 확인해야 합니다.

   따라서 **실제 분포의 평가와 예외 상황의 평가를 함께 보고**, 일반 성능과 예외 대응 사이의 균형에 맞춰 학습 비율을 조정해야 합니다.

5. **Self-Instruct를 “스스로 계속 학습하는 모델”이라고 설명하면, 데이터 생성과 모델 학습 사이의 절차가 생략됩니다.**

   Self-Instruct는 사람이 작성한 소수의 초기 지시문을 바탕으로 모델이 새로운 지시문과 입력·출력 예시를 생성하고, 부적절하거나 유사한 예시를 걸러 학습 데이터를 확보한 뒤, 그 데이터로 모델을 미세조정하는 방식입니다. [논문 요약](/notes/synthetic-data-study/01-foundations/papers/wang-2023/)

   여기서 **데이터 수집 단계**는 학습 예시를 생성·선별하는 과정이며, 이 과정만으로 모델의 가중치가 바뀌지는 않습니다. **미세조정 단계**에서 별도의 학습을 실행해야 가중치가 갱신됩니다.

   따라서 “모델이 생성한 데이터를 선별해 자신의 미세조정에 활용하는 방법”이라고 설명하는 편이 정확합니다. 데이터 생성의 반복이 곧 실시간·지속적인 모델 학습을 뜻하지는 않습니다.
...
[다음 강의 → 2강 · 텍스트·지시문 생성과 데이터셋 설계](/notes/synthetic-data-study/02-text-instructions/)
